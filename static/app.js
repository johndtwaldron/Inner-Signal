...
function silentAudioUrl(){const samples=800,buffer=new ArrayBuffer(44+samples*2),view=new DataView(buffer),write=(offset,text)=>[...text].forEach((char,index)=>view.setUint8(offset+index,char.charCodeAt(0)));write(0,"RIFF");view.setUint32(4,36+samples*2,true);write(8,"WAVEfmt ");view.setUint32(16,16,true);view.setUint16(20,1,true);view.setUint16(22,1,true);view.setUint32(24,8000,true);view.setUint32(28,16000,true);view.setUint16(32,2,true);view.setUint16(34,16,true);write(36,"data");view.setUint32(40,samples*2,true);return URL.createObjectURL(new Blob([buffer],{type:"audio/wav"}))}
async function play(id){
  ...
  el.onloadstart=()=>diagnostic("media-loadstart",{source});el.onloadedmetadata=()=>{diagnostic("media-metadata",{duration:el.duration,readyState:el.readyState,networkState:el.networkState,source});if(source!=="drive-direct"&&Number.isFinite(el.duration)){item.duration_seconds=el.duration;knownDurations[item.id]=el.duration;localStorage.setItem("inner-signal-durations",JSON.stringify(knownDurations));$("#duration").textContent=fmt(el.duration);renderLibrary()}};el.oncanplay=()=>diagnostic("media-canplay",{readyState:el.readyState});el.onplaying=()=>diagnostic("media-playing",{currentTime:el.currentTime});el.onwaiting=()=>diagnostic("media-waiting",{readyState:el.readyState,networkState:el.networkState});el.onstalled=()=>diagnostic("media-stalled",{readyState:el.readyState,networkState:el.networkState});
  if(HOSTED){try{if(source==="offline-cache"){el.src=new URL(`offline/${id}`,location.href).href}else{const silent=silentAudioUrl(),originalVolume=el.volume;let loggedProgress=-10;el.volume=0;el.loop=true;el.src=silent;el.play().catch(()=>{});$("#now-title").textContent=`Loading 0% · ${item.title}`;diagnostic("buffer-start",{id,expected:item.size_bytes,mime:item.mime_type});const mediaUrl=await DriveSource.bufferedUrl(item,progress=>{if(request!==playbackRequest)return;const amount=progress.percent==null?size(progress.received):`${progress.percent}%`;$("#now-title").textContent=`Loading ${amount} · ${item.title}`;if(progress.percent===100||progress.percent==null||progress.percent>=loggedProgress+10){loggedProgress=progress.percent??loggedProgress;diagnostic("buffer-progress",{received:progress.received,total:progress.total,percent:progress.percent,contentType:progress.contentType,cached:Boolean(progress.cached)})}});URL.revokeObjectURL(silent);if(request!==playbackRequest)return;el.pause();el.loop=false;el.volume=originalVolume;el.src=mediaUrl;source="drive-buffered";diagnostic("buffer-complete",{id,size:item.size_bytes,mime:item.mime_type});$("#now-title").textContent=item.title}}catch(error){diagnostic("source-error",{name:error.name,message:error.message});$("#play").textContent="▶";$("#now-title").textContent=item.title;$("#drive-status").textContent=`Could not prepare this recording: ${error.message}`;return}}
  el.load();el.play().then(()=>diagnostic("play-promise-resolved",{source})).catch(error=>{diagnostic("play-promise-rejected",{name:error.name,message:error.message,code:el.error?.code||null,readyState:el.readyState,networkState:el.networkState});$("#play").textContent="▶";$("#drive-status").textContent=`Playback was blocked: ${error.message}. Tap Play once more; if it still fails, share the diagnostic file.`});
